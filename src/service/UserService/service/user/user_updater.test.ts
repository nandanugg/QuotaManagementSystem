...
t.test(
  "should call UpdateUserQuota with correct user and handle success",
  async () => {
    const user: User = {
      email: "",
      id: "123",
    ...
    const UpdateUserQuotaStub = sinon
      .stub(deps, "UpdateUserQuota")
      .resolves(null);
    deps.UpdateUserQuota = UpdateUserQuotaStub;

    const srv = UpdateUserQuota({
      ...deps,
      UpdateUserQuota: UpdateUserQuotaStub,
    });
